...
public class GameManager : MonoBehaviour
{
    ...
    private void Update()
    {
        ...
        if (playerReady)
        {
            ...
            //文字点滅
            screenMessageText.color = GetAlphaColor(screenMessageText.color);
    ...
    private void FreezePlayer(bool freeze)
    {
        ...
        if (player != null)
        {
            player.GetComponent<PlayerController2D>().FreezeInput(freeze);
            player.GetComponent<PlayerController2D>().FreezePlayer(freeze);
        }
    }
    ...
    {
        textTime += Time.deltaTime * 5.0f * textSpeed;
        color.a = Mathf.Sin(textTime) * 0.5f + 0.5f;
        return color;
    }
}